...
public class ViewModel {
     ...
     */
    private final StringProperty cargoManifestText =
            new SimpleStringProperty("Cargo Manifest");
    ...
    /**
     * Creates a new view model and constructs a port by reading from the given
     * filenames.
     *
     * @param filename filename specifying the path to: the port file
     *
     * @throws IOException          if loading from the file specified generates
     *                              an IOException
     * @throws BadEncodingException if the file is invalid according to
     *                              {@link Port#initialisePort(Reader)}
     *
     * @require filename != null &amp;&amp; filenames.size() == 1
     */
     ...
     * button is clicked.
     * <p>
     * The overall purpose of this event handler is to update the
     * <pre>cargoManifestText</pre> string property with the string
     * representation of the cargo on the currently selected ship, as
     * returned by {@link BulkCarrier#getCargo()} or
     * {@link ContainerShip#getCargo()}.
     * <p>
     * The event handler should perform the following actions:
     * <ol>
     *     <li>If no ship is currently selected (i.e.
     *     {@link #getSelectedShip()} ()} is storing null), then the event
     *     handler should return immediately without taking any further
     *     action.</li>
     *     <li>If calling {@link BulkCarrier#getCargo()} or
     ...
     *     For Example:<pre>BulkCargo 42 to Brazil [OIL - 420]</pre></li>
     *     <li>If the ship currently selected  is a {@link ContainerShip}, the
     *     <pre>cargoManifestText</pre> property should be set to a String
     *     containing the {@link Container#toString()} representation of each
     *     of the cargo onboard separated by a comma and
     *     {@link System#lineSeparator()}.<br>
     *     <b>Note: The last line should not have a trailing comma</b><br> For
     *     Example: <pre>Container 43 to Australia [OTHER]</pre>,
    ...
    /**
     * The purpose of this method is to update the <pre>evaluatorsText</pre>
     * string. Called each {@link #elapseOneMinute()}
     * <p>
     ...
     *                     <li>The current number of Quay's occupied should be
     *                     printed as the following format <pre>num Quay(s)
     *                     currently occupied</pre> where num is the number of
     *                     occupied quays</li>
     *                 </ul>
     *             </li>
     ...
     *                 <ul>
     *                     <li>The name of the evaluator</li>
     *                     <li>For each unique Country flag that the evaluator
     *                     has seen it should print that statistic in the
     *                     following format <pre>country : num</pre> where
     *                     country is the name of the country flag and num is
     *                     the number of times that flag has been seen</li>
     *                 </ul>
     *             </li>
     *             <li>If the port has a {@link ShipThroughputEvaluator}:
     *                 <ul>
     *                     <li>The name of the evaluator</li>
     *                     <li>The current number of ships that have passed
     *                     through the port in the last hour should be
     *                     printed as the following format <pre>num Ships
     *                     passed in the last hour</pre> where num is the number
     *                     of ships through the port in the last hour</li>
     *                 </ul>
     *             </li>
     *             <li>If the port has a {@link CargoDecompositionEvaluator}:
     *                 <ul>
     *                     <li>The name of the evaluator</li>
     *                     <li>For each unique cargo type that the evaluator has
     *                     seen it should print that statistic in the following
     *                     format <pre>cargo : num</pre> where cargo is the name
     *                     of the cargo class and num is the number of times
     *                     that flag has been seen</li>
     *                 </ul>
     *             </li>
     *         </ol>
     *     </li>
            ...
            for (StatisticsEvaluator eval : port.getEvaluators()) {
                ...
                switch (eval.getClass().getSimpleName()) {
                    ...
                    case "ShipFlagEvaluator":
                        ShipFlagEvaluator flagEval = (ShipFlagEvaluator) eval;
                        Map<String, Integer> distribution =
                                flagEval.getFlagDistribution();
                    ...
                    case "CargoDecompositionEvaluator":
                        CargoDecompositionEvaluator cargoEval =
                                (CargoDecompositionEvaluator) eval;
                        Map<String, Integer> decomposition =
                                cargoEval.getCargoDistribution();
     ...
     *     <li>EncodedCargo is the encoded representation of each individual
     *     cargo in the simulation</li>
     *     <li>numShips is the total number of ships in the simulation</li>
     *     <li>EncodedShip is the encoded representation of each individual ship
     *     encoding in the simulation</li>
     *     <li>numQuays is the total number of quays in the Port</li>
     *     <li>EncodedQuay is the encoded representation of each individual quay
     *     in the simulation</li>
     *     <li>numShipsInQueue is the total number of ships in the ship queue in
     *     the port</li>
     ...
     *     in Port</li>
     *     <li>EncodedMovement is the encoded representation of each individual
     *     Movement in the aforementioned list</li>
     *     <li>numEvaluators is the number of statistics evaluators in the
     *     Port</li>
     *     <li>EvaluatorSimpleName is the name given by
     *     {@link Class#getSimpleName()} for each evaluator in the
    ...
    /**
     * Saves the current state of the port simulation to the same file it was
     * loaded from when the application was launched.
     *
    ...
    /**
     * Returns whether or not the state of the model has changed since it was
     * last checked for a change.
     *
    ...
    /**
     * Returns the property storing the contents of the selected ships cargo
     * info text box.
     *
    ...
    /**
     * Returns the property storing the text appended to window title when the
    ...
    /**
     * Returns the property storing the text shown for the pause/unpause menu
     * item.
     *
